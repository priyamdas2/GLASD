...
% Define inline ackley with counter
ackley_eval = @(x) eval_count_store(ackley(x));
ackley_eval_ga = @(x) arrayfun(@(i) eval_count_store(ackley(x(i,:)')), 1:size(x,1))';

% GLASD
eval_count = 0;
params.T = round(3000 * log(n));

[xg, fg, histg] = GLASD(ackley_eval, lb, ub, x0, params);
...
[xs, fs] = simulannealbnd(ackley_eval, x0, lb, ub, options_sa);
...
[xga, fga] = ga(ackley_eval_ga, n, [], [], [], [], lb, ub, [], options_ga);
...
[xpso, fpso] = particleswarm(ackley_eval_ga, n, lb, ub, options_pso);
...
options_ps = optimoptions('patternsearch', ...
    ...
    'MaxIterations', params.T, ...
    'PlotFcn', {});
[xps, fps] = patternsearch(ackley_eval, x0, [], [], [], [], lb, ub, options_ps);
...
options_fmc1 = optimoptions('fmincon', ...
    ...
    'MaxIterations', params.T);
[xfmc1, ffmc1] = fmincon(ackley_eval, x0, [], [], [], [], lb, ub, [], options_fmc1);
...
options_fmc2 = optimoptions('fmincon', ...
    ...
    'Algorithm','sqp', ...
    'MaxIterations', params.T);
[xfmc2, ffmc2] = fmincon(ackley_eval, x0, [], [], [], [], lb, ub, [], options_fmc2);
...
options_fmc3 = optimoptions('fmincon', ...
    ...
    'MaxIterations', params.T);
[xfmc3, ffmc3] = fmincon(ackley_eval, x0, [], [], [], [], lb, ub, [], options_fmc3);
...
title('Ackley', 'FontSize', 16, 'FontWeight', 'bold');
xlim([0 4000]);
grid on;

% Save plot WITHOUT legend
print(gcf, 'plot_ackley', '-dpng', '-r600');  % 600 DPI PNG
