...
% Define inline griewank with counter
griewank_eval = @(x) eval_count_store(griewank(x));
griewank_eval_ga = @(x) arrayfun(@(i) eval_count_store(griewank(x(i,:)')), 1:size(x,1))';
% GLASD
eval_count = 0;
params.T = round(3000 * log(n));

[xg, fg, histg] = GLASD(griewank_eval, lb, ub, x0, params);
...
[xs, fs] = simulannealbnd(griewank_eval, x0, lb, ub, options_sa);
...
[xga, fga] = ga(griewank_eval_ga, n, [], [], [], [], lb, ub, [], options_ga);
...
[xpso, fpso] = particleswarm(griewank_eval_ga, n, lb, ub, options_pso);
...
options_ps = optimoptions('patternsearch', ...
    ...
    'MaxIterations', params.T, ...
    'PlotFcn', {});
[xps, fps] = patternsearch(griewank_eval, x0, [], [], [], [], lb, ub, options_ps);
...
options_fmc1 = optimoptions('fmincon', ...
    ...
    'MaxIterations', params.T);
[xfmc1, ffmc1] = fmincon(griewank_eval, x0, [], [], [], [], lb, ub, [], options_fmc1);
...
options_fmc2 = optimoptions('fmincon', ...
    ...
    'Algorithm','sqp', ...
    'MaxIterations', params.T);
[xfmc2, ffmc2] = fmincon(griewank_eval, x0, [], [], [], [], lb, ub, [], options_fmc2);
...
options_fmc3 = optimoptions('fmincon', ...
    ...
    'MaxIterations', params.T);
[xfmc3, ffmc3] = fmincon(griewank_eval, x0, [], [], [], [], lb, ub, [], options_fmc3);
...
title('Griewank', 'FontSize', 16, 'FontWeight', 'bold');
xlim([0 4000]);
grid on;

% Save plot WITHOUT legend
print(gcf, 'plot_griewank', '-dpng', '-r600');  % 600 DPI PNG
