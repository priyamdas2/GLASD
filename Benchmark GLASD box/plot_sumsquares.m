...
% Define inline sumsquares with counter
sumsquares_eval = @(x) eval_count_store(sumsquares(x));
sumsquares_eval_ga = @(x) arrayfun(@(i) eval_count_store(sumsquares(x(i,:)')), 1:size(x,1))';
% GLASD
eval_count = 0;
params.T = round(3000 * log(n));

[xg, fg, histg] = GLASD(sumsquares_eval, lb, ub, x0, params);
...
[xs, fs] = simulannealbnd(sumsquares_eval, x0, lb, ub, options_sa);
...
[xga, fga] = ga(sumsquares_eval_ga, n, [], [], [], [], lb, ub, [], options_ga);
...
[xpso, fpso] = particleswarm(sumsquares_eval_ga, n, lb, ub, options_pso);
...
options_ps = optimoptions('patternsearch', ...
    ...
    'MaxIterations', params.T, ...
    'PlotFcn', {});
[xps, fps] = patternsearch(sumsquares_eval, x0, [], [], [], [], lb, ub, options_ps);
...
options_fmc1 = optimoptions('fmincon', ...
    ...
    'MaxIterations', params.T);
[xfmc1, ffmc1] = fmincon(sumsquares_eval, x0, [], [], [], [], lb, ub, [], options_fmc1);
...
options_fmc2 = optimoptions('fmincon', ...
    ...
    'Algorithm','sqp', ...
    'MaxIterations', params.T);
[xfmc2, ffmc2] = fmincon(sumsquares_eval, x0, [], [], [], [], lb, ub, [], options_fmc2);
...
options_fmc3 = optimoptions('fmincon', ...
    ...
    'MaxIterations', params.T);
[xfmc3, ffmc3] = fmincon(sumsquares_eval, x0, [], [], [], [], lb, ub, [], options_fmc3);
...
title('Sumsquares', 'FontSize', 16, 'FontWeight', 'bold');
xlim([0 4000]);
grid on;

% Save plot WITHOUT legend
print(gcf, 'plot_sumsquares', '-dpng', '-r600');  % 600 DPI PNG
