...
% UNOBSERVED FACTOR VERSION (Latent F)
...
for rep = 1:num_exp
    ...
    % ---- Fit FWZ (LATENT FACTORS) on Setting 1 ----
    fit1 = fan2019_cov_estimator_unobservedF(Y1, r, opts);
    ...
    fname_err1 = sprintf('Output/FWZ_latentF_errors_%s_setting1_elliptical_p_%d_nu_%d_rep_%d.csv', ...
        Sigma_u_regime, p, nu, rep);
    ...
    % ---- Fit FWZ (LATENT FACTORS) on Setting 2 ----
    fit2 = fan2019_cov_estimator_unobservedF(Y2, r, opts);
    ...
    fname_err2 = sprintf('Output/FWZ_latentF_errors_%s_setting2_componentwise_p_%d_nu_%d_rep_%d.csv', ...
        Sigma_u_regime, p, nu, rep);
    ...
    disp('Robust POET (Latent-F) error files written:');
    disp(['  ', fname_err1]);
    disp(['  ', fname_err2]);
end