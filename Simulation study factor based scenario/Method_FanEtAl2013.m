%% ============================================
% Classical POET (2013) Model Fitting & Errors
% Computes error metrics for POET estimator only
% ============================================

clear; clc;

addpath('./FanEtAl2013/');   % contains poet2013_cov_estimator.m
addpath('./supp funs/');
...
nu  = 3;
num_exp = 10;
Sigma_u_regime = 'diagonal';   % 'diagonal' / 'random'
...
for rep = 1:num_exp
    ...
    % ---- Fit classical POET on setting 1 ----
    fit1 = poet2013_cov_estimator(Y1, r, opts);
    
    Sigma_hat_1   = (fit1.Sigma_hat   + fit1.Sigma_hat')   / 2;
    Sigma_u_hat_1 = (fit1.Sigma_u_hat + fit1.Sigma_u_hat') / 2;
    
    % ---- Inversion (no artificial regularization) ----
    ...
    fname_err1 = sprintf('Output/POET_errors_%s_setting1_elliptical_p_%d_nu_%d_rep_%d.csv', ...
        Sigma_u_regime, p, nu, rep);
    ...
    % ---- Fit classical POET on setting 2 ----
    fit2 = poet2013_cov_estimator(Y2, r, opts);
    ...
    fname_err2 = sprintf('Output/POET_errors_%s_setting2_componentwise_p_%d_nu_%d_rep_%d.csv', ...
        Sigma_u_regime, p, nu, rep);
    ...
    disp('Classical POET error files written:');
    disp(['  ', fname_err1]);
    disp(['  ', fname_err2]);
